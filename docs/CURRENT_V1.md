...
- HiSpeed now includes a preview Payment Request presentation that groups each selected vehicle's plan schedule by collection milestone: secure vehicle, before shipment, and destination/gateway milestone for Flex. It shows customer-facing request totals and per-vehicle line amounts in the selected presentation currency.
