...
The Owner no longer needs to approve every sourced vehicle individually. A new import may become eligible for automatic Browse publication only when deterministic gates all pass: the source URL/listing ID is retained; duplicate checks pass; the listing and every displayed photo belong to the same vehicle; the accessible source gallery has been captured with more than one genuine photo; no synthetic/duplicated filler image is used; plates are not blurred; required vehicle facts and source price have evidence; customer-visible media contains no seller contact or internal data; and no material conflict or missing required field remains. Any failed or uncertain gate stays `NEEDS_REVIEW` and is the only class requiring human approval. This decision does not authorize seller contact, availability confirmation, negotiation, reservation, purchase, or payment. Existing fail-closed publication gates remain in force.

### Xiangshihai shared-data storefront (2026-09-17)

`xiangshihai.com` is a host-aware Chinese-first marketplace storefront over the same Buying Browser inventory, media, Vehicle Cases, sourcing adapters, and Owner operations used by `nkautotrade.com`. It uses a distinct orange high-density marketplace presentation inspired by familiar cross-border commerce patterns without copying third-party trademarks or layouts. Host routing changes branding and customer presentation only; it does not duplicate vehicle truth. Customer USD offers lock the observed market FX, the deterministic `ceil(marketRate)+1` customer rate, the THB equivalent, source, and date. Offers enter `Awaiting NK Review` and are never sent to a seller automatically.

### Controlled Seller Relay (2026-09-16)

Vehicle Cases now support a distinct Seller Relay queue. A customer can ask NK to relay factual questions about availability, current price, mileage, documents, or vehicle facts. The request is recorded separately from NK AI's grounded local answers, translated/prepared for Thai review where deterministic templates apply, and shown in the authenticated Owner/Jaklaen Case queue. Requests that attempt to negotiate, reserve, purchase, deposit, transfer, or pay are blocked and never become send-ready. Customer writes cannot forge Sent or Seller Replied states. Actual external sending and reply capture remain disabled until the connector send path is separately tested and the Owner approves the first live message; Login/MFA/CAPTCHA must stop the workflow.

### First-party conversion measurement (2026-09-16)

Buying Browser workspaces now retain a bounded first-party conversion event stream for vehicle saves, Vehicle Case creation, availability requests, inspection requests, quotation requests, and Wanted Request creation. Events use no paid third-party analytics service, sync with authenticated workspaces, remain device-local for guests, and are capped at 500 records. This is measurement only: it does not contact sellers, issue quotations, purchase vehicles, or authorize payments.

### Evidence-backed specification extraction (2026-09-15)

Vehicle imports now build per-field specification evidence from structured listing fields, title, and seller description before any photo inference. Each field records its source, confidence, and status (`confirmed`, `inferred`, `unknown`, or `conflict`). Unknown values never count as confirmed customer matches. Photo-derived facts may fill an unknown field only after image review; disagreement with listing evidence becomes `conflict` and requires review. Image publication also requires minimum gallery coverage plus duplicate and unrelated-image checks. Customer vehicle pages expose this provenance and image-review status without exposing seller identity or source URLs.
