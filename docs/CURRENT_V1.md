...
Customer mobile visual direction: the active `/buy` customer screens now use an Owner-approved readability-first Marketplace-style pass. On iPhone-sized screens Browse uses larger one-column image-first vehicle cards instead of dense two-column cards, with larger price/title/meta text, larger filter/action touch targets, and a larger bottom navigation. Vehicle Detail keeps the shorter mobile source-price copy, top-level Check Availability / Ask NK AI / Request Inspection actions, no bottom-navigation overlap, and larger specification/summary text. My Cases, Account, chat, pricing, quotation, and PI customer surfaces inherit larger mobile text and touch targets. The underlying Vehicle Case, pricing, inspection, localization, source adapter, and safety boundaries remain unchanged.
