...
export function createCustomerOffer(caseRecord, offerUsd, fxQuote, now = new Date()) {
  const usd = Math.round(Number(offerUsd));
  const marketRate = Number(fxQuote?.marketRate);
  const customerRate = Number(fxQuote?.customerRate);
  if (!Number.isFinite(usd) || usd <= 0 || !Number.isFinite(marketRate) || marketRate <= 0 || !Number.isFinite(customerRate) || customerRate <= 0) return caseRecord;
  const createdAt = safeTime(now);
  const offer = { id: `${caseRecord.id}-offer-${createdAt}`, offerUsd: usd, offerThb: Math.round(usd * customerRate), marketRateThbPerUsd: marketRate, customerRateThbPerUsd: customerRate, fxRule: "ceil(marketRate)+1", fxSource: String(fxQuote?.source || "Unknown FX source"), fxRateDate: fxQuote?.rateDate || null, status: "Awaiting NK Review", createdAt };
  return { ...caseRecord, updatedAt: createdAt, customerOfferRequests: [...(caseRecord.customerOfferRequests || []), offer], messages: [...caseRecord.messages, { id: `${offer.id}-customer`, sender: "Customer", text: `I would like NK to ask the seller about an offer of USD ${usd.toLocaleString("en-US")} (THB ${offer.offerThb.toLocaleString("en-US")} using NK customer FX ${customerRate}).`, createdAt, delivery: "Recorded - not sent to seller" }, { id: `${offer.id}-system`, sender: "System", text: "Offer request recorded for NK review. This is not a reservation, purchase agreement, or authorization to pay. NK has not sent it to the seller yet.", createdAt, delivery: "Recorded" }], timeline: [...caseRecord.timeline, { id: `${offer.id}-timeline`, title: "Customer offer requested", detail: `USD ${usd.toLocaleString("en-US")} converted to THB ${offer.offerThb.toLocaleString("en-US")} at locked customer FX ${customerRate}; awaiting NK approval before seller contact.`, createdAt }] };
}
