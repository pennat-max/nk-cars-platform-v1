...
import { ClipboardCheck, FolderKanban, Globe2, Heart, MapPin, MessageSquare, Search, UserRound } from "lucide-react";
...
const navItems = [
  ...
  { key: "inspections", labelKey: "inspections", href: "/buy/inspections", icon: ClipboardCheck },
    ...
    <div className="buying-browser" data-buying-browser-v1>
      <header className="bb-header">
        <div className="bb-header-inner">
          ...
          <div className="bb-market-location" aria-label="Vehicle search location"><MapPin size={16} /><span><small>{t("searchArea")}</small><b>{t("thailand")}</b></span></div>
          <nav className="bb-desktop-nav" aria-label="Buying Browser navigation">
            {navItems.map((item) => { const Icon = item.icon; return <Link key={item.key} className={selected === item.key ? "active" : ""} href={item.href}><Icon size={17} />{t(item.labelKey)}</Link>; })}
          </nav>
      ...
      <nav className="bb-bottom-nav" aria-label="Buying Browser mobile navigation">
        {navItems.map((item) => { const Icon = item.icon; const count = item.key === "saved" ? state.savedListingIds.length : item.key === "cases" ? state.cases.length : 0; return <Link key={item.key} className={selected === item.key ? "active" : ""} href={item.href}><span><Icon size={21} />{count > 0 && <i>{count > 9 ? "9+" : count}</i>}</span><b>{t(item.labelKey)}</b></Link>; })}
      </nav>
    </div>
  );
}
