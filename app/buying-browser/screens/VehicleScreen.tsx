  ...
  return (
    <>
        ...
        <section className="bb-gallery">
          <div className="bb-gallery-stage" data-swipe-gallery>
            <div ref={galleryTrack} className="bb-gallery-track" onScroll={(event) => { const width = event.currentTarget.clientWidth; if (width) setImageIndex(Math.round(event.currentTarget.scrollLeft / width)); }}>
              {listing.imageUrls.map((image, index) => <div className="bb-gallery-slide" key={image}><button type="button" className="bb-gallery-open" onClick={() => openFullscreen(index)} aria-label={t("openPhoto", { number: index + 1 })}><VehiclePhoto listing={listing} imageUrl={image} alt={`${listing.title} view ${index + 1}`} /></button></div>)}
            </div>
            <span className="bb-gallery-counter">{t("photoCounter", { current: imageIndex + 1, total: listing.imageUrls.length })}</span>
            ...
            {listing.imageUrls.length > 1 && <><button className="bb-gallery-arrow previous" onClick={() => showPhoto(imageIndex - 1)} disabled={imageIndex === 0} aria-label={t("previousPhoto")} title={t("previousPhoto")}><ChevronLeft size={21} /></button><button className="bb-gallery-arrow next" onClick={() => showPhoto(imageIndex + 1)} disabled={imageIndex === listing.imageUrls.length - 1} aria-label={t("nextPhoto")} title={t("nextPhoto")}><ChevronRight size={21} /></button></>}
          </div>
      ...
      {fullscreenOpen && <section className="bb-photo-viewer" role="dialog" aria-modal="true" aria-label={`${listing.title} photo gallery`} data-fullscreen-viewer>
        <header>
          <button type="button" onClick={closeFullscreen} aria-label={t("closeGallery")} title={t("closeGallery")} autoFocus><X size={30} /></button>
          <strong aria-live="polite">{t("photoCounter", { current: imageIndex + 1, total: listing.imageUrls.length })}</strong>
        </header>
        ...
        {listing.imageUrls.length > 1 && <><button type="button" className="bb-photo-viewer-arrow previous" onClick={() => showFullscreenPhoto(imageIndex - 1)} disabled={imageIndex === 0} aria-label={t("previousPhoto")}><ChevronLeft size={30} /></button><button type="button" className="bb-photo-viewer-arrow next" onClick={() => showFullscreenPhoto(imageIndex + 1)} disabled={imageIndex === listing.imageUrls.length - 1} aria-label={t("nextPhoto")}><ChevronRight size={30} /></button></>}
      </section>}
