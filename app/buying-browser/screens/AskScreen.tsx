  ...
  return (
    <>
      ...
      <section className="bb-ai-search-tool">
        ...
        <div className="bb-prompt-chips"><button onClick={() => setQuestion("Toyota Hilux Revo 2020-2023, 4WD, AT, Double Cab, under USD 27,200")}>Revo 4WD AT</button><button onClick={() => setQuestion("Ford Ranger 2020-2022, 4WD, AT, Bangkok or Central Thailand")}>Ranger in Central</button><button onClick={() => setQuestion("Reliable Double Cab pickup under USD 20,000")}>Pickup under USD 20k</button></div>
        <form onSubmit={submit}><textarea value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Example: Find a 2020-2022 Hilux Revo, 4WD, AT, Double Cab, under USD 27,200" aria-label="Describe the vehicle you want" /><button className="bb-button primary" type="submit"><Send size={18} />Search with NK AI</button></form>
