...
export function AccountScreen() {
  ...
  return (
    <>
      ...
      <section className="bb-account-section"><div className="bb-section-heading"><div><p className="bb-kicker">Vehicle inventory</p><h2>{t("inventoryStorage")}</h2></div><span className={sourceStatus.live ? "bb-status-chip requested" : "bb-status-chip pending"}>{sourceStatus.live ? <Database size={13} /> : <WifiOff size={13} />}{t(sourceStatus.live ? "synchronized" : "fallbackActive")}</span></div><div className="bb-source-account-row"><div><b>{sourceStatus.label}</b><p>{sourceStatus.message}</p></div><a className="bb-button secondary" href="https://www.facebook.com/marketplace/" target="_blank" rel="noreferrer">Open source app/browser<ExternalLink size={16} /></a></div><p className="bb-security-note"><LockKeyhole size={16} />{t("inventoryPrivacy")}</p></section>
      <section className="bb-account-section"><div className="bb-section-heading"><div><p className="bb-kicker">Customer workspace</p><h2>{customer.isPreview ? "This device" : "Secure account data"}</h2></div><span className={`bb-status-chip ${workspaceSync.mode === "synced" ? "market" : workspaceSync.mode === "syncing" ? "requested" : "pending"}`}>{workspaceSync.mode === "synced" ? <Database size={13} /> : workspaceSync.mode === "syncing" ? <Clock3 size={13} /> : <WifiOff size={13} />}{workspaceSync.mode}</span></div><p className="bb-workspace-status">{workspaceSync.message}</p><dl className="bb-preview-stats"><div><dt>Saved vehicles</dt><dd><Heart size={17} />{state.savedListingIds.length}</dd></div><div><dt>Vehicle Cases</dt><dd><FolderKanban size={17} />{state.cases.length}</dd></div><div><dt>Inspection requests</dt><dd><ClipboardCheck size={17} />{state.cases.filter((item) => item.inspectionQuote?.status === "Requested - Awaiting Provider").length}</dd></div></dl>{customer.isPreview && <Link className="bb-button primary" href="/signin-with-chatgpt?return_to=%2Fbuy%2Faccount"><UserRound size={17} />Sign in to sync Vehicle Cases</Link>}{confirmReset ? <div className="bb-reset-confirm" role="group" aria-label="Confirm workspace data reset"><p>Remove saved vehicles, Vehicle Cases, and conversation history from this {customer.isPreview ? "device" : "account workspace"}?</p><div><button className="bb-button secondary" onClick={() => setConfirmReset(false)}>Cancel</button><button className="bb-button danger" onClick={reset}><RotateCcw size={17} />Clear workspace data</button></div></div> : <button className="bb-button danger" onClick={() => setConfirmReset(true)}><RotateCcw size={17} />Reset {customer.isPreview ? "device" : "account"} data</button>}</section>
      <p className="bb-honesty-note"><ShieldCheck size={16} />{customer.isPreview ? "Anonymous activity stays on this device. Sign in before creating business-critical Vehicle Cases." : "Vehicle Cases and conversation history are stored in an account-isolated server workspace with revision checks and append-only sync events."}</p>
    </>
  );
}
