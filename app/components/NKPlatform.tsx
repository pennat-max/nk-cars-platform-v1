...
import { buildCustomerDescriptionEn, defaultSource, markup, profit, sanitizeCustomerDescriptionEn, thb } from "../lib/domain";
...
export default function NKPlatform({
  ...
  useEffect(() => {
    const persistableVehicles = vehicles.map((vehicle) => {
      const remoteImages = vehicle.images?.filter((image) => image.startsWith("https://"));
      return { ...vehicle, images: remoteImages?.length ? remoteImages : undefined };
    });
...
function CustomerGallery({ vehicle }: { vehicle: Vehicle }) {
  const images = [...new Set((vehicle.images?.length ? vehicle.images : [vehicle.coverImage || vehicle.image]).filter(Boolean))].slice(0, 30);
  const [activeImage, setActiveImage] = useState(images[0] || vehicle.image);
  return <section>
    <div className="detail-image"><img src={activeImage} alt={`${vehicle.year} ${vehicle.brand} ${vehicle.model}`} /><StatusBadge state={vehicle.state === "Published" ? "Available" : vehicle.state}/></div>
    {images.length > 1 ? <div className="thumb-row" aria-label="Vehicle photo gallery">{images.map((image, index) => <button className={activeImage === image ? "selected" : ""} key={`${image}-${index}`} onClick={() => setActiveImage(image)} aria-label={`Show vehicle photo ${index + 1} of ${images.length}`}><img src={image} alt={`Vehicle view ${index + 1}`} /></button>)}</div> : null}
    <small className="gallery-count">{images.length} photo{images.length === 1 ? "" : "s"}</small>
  </section>;
}

function Detail({vehicle,internal,chat,chatInput,setChatInput,sendChat,go}:{vehicle:Vehicle;internal:boolean;chat:{from:"ai"|"user";text:string}[];chatInput:string;setChatInput:(s:string)=>void;sendChat:()=>void;go:(v:View,id?:string)=>void}) {
  const publicDescription = sanitizeCustomerDescriptionEn(vehicle.customerDescriptionEn) || buildCustomerDescriptionEn(vehicle);
  return <><button className="back" onClick={()=>go("marketplace")}>← Back to Marketplace</button><div className="detail-layout"><CustomerGallery vehicle={vehicle}/><section className="detail-copy"><p className="eyebrow">{vehicle.stockNo} · DEMO DATA</p><h1>{vehicle.year} {vehicle.brand}<br/>{vehicle.model}</h1><strong className="detail-price">{thb(vehicle.state==="Sold"?(vehicle.lastSoldPrice||vehicle.sellingPrice):vehicle.sellingPrice)}</strong>{!vehicle.availabilityVerified&&<div className="warning">Availability requires staff verification</div>}<dl className="specs">{[["Engine",vehicle.engine],["Transmission",vehicle.transmission],["Drive",vehicle.drive],["Body",vehicle.body],["Mileage",vehicle.mileage],["Color",vehicle.color],["Plate",vehicle.plateMasked],["VIN",vehicle.vinMasked]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl><section className="customer-description"><p className="eyebrow">Vehicle Overview</p><p>{publicDescription}</p></section><div className="detail-actions"><button className="button primary" onClick={()=>go("inquiry",vehicle.id)}>I&apos;m Interested</button><button className="button secondary" onClick={()=>go("inquiry",vehicle.id)}>Check Availability</button><button className="text-button" onClick={()=>go("wanted",vehicle.id)}>Find Similar</button>{internal&&<button className="text-button" onClick={()=>go("vehicle360",vehicle.id)}>Open internal Vehicle 360 →</button>}</div></section></div><section className="ai-panel"><div className="ai-heading"><span>✦</span><div><p className="eyebrow light">Context-aware sales assistant</p><h2>Ask NK AI about this {vehicle.model}</h2></div></div><div className="chat-log">{chat.map((m,i)=><div className={`message ${m.from}`} key={i}>{m.text}</div>)}</div><div className="chat-input"><input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendChat()} placeholder="Is this vehicle available?"/><button onClick={sendChat}>Send</button></div><small>AI will not guess shipping, discounts, price or unverified availability.</small></section></>;
}
